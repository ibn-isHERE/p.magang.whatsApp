...
const userState = {}; // To store user chat states
const chatSessions = {}; // To store chat session timeouts
...
client.on('message', async (message) => {
    try {
        ...
        const menu = {
            '1': 'Informasi Produk A.\nAnda bisa mendapatkan informasi lebih lanjut di website kami: [link produk A]',
            '2': 'Informasi Produk B.\nAnda bisa mendapatkan informasi lebih lanjut di website kami: [link produk B]',
            '3': 'Informasi Pengiriman.\nPengiriman kami lakukan setiap hari kerja.',
            '4': 'Chat dengan Admin.',
        };

        const welcomeMessage = `
Selamat datang di Customer Service kami!
Silakan pilih salah satu opsi di bawah ini dengan mengetikkan nomornya:
...
`;
        ...
        if (chatSessions[fromNumber]) {
            clearTimeout(chatSessions[fromNumber]);
        }
        chatSessions[fromNumber] = setTimeout(() => {
            delete userState[fromNumber];
            console.log(`Sesi untuk ${fromNumber} telah berakhir.`);
        }, 10 * 60 * 1000); // 10 minutes timeout

        if (!userState[fromNumber] || userState[fromNumber] === 'MENU_UTAMA') {
            if (['halo', 'hi', 'menu'].includes(messageBody)) {
            ...
            if (menu[messageBody]) {
                if (messageBody === '4') {
                    ...
                    client.sendMessage(message.from, 'Anda sekarang terhubung dengan admin. Silakan sampaikan pertanyaan Anda.');
                    // Notify admin on web interface
